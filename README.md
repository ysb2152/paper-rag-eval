...
- **접근:** 검색 평가(recall@k·MRR·nDCG) + 답변 평가(faithfulness·correctness·환각률, LLM-as-judge) + 청킹/임베딩/리랭커/프롬프트 **ablation** + **회귀 테스트(CI식)** 를 1급 시민으로.
- **스택:** 로컬 GPU 임베딩·리랭커(bge-m3 / bge-reranker, RTX 3080) + FAISS + Claude API(생성·judge) + FastAPI.
